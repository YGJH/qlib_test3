...
import math
import qlib
from torch.cuda.amp import GradScaler, autocast  # ensure autocast import
...
class TransformerModel(Model):
    def __init__(
        ...
        **kwargs,
        ...
        if optimizer.lower() == "adam":
            ...
            self.train_optimizer = optim.Adagrad(self.model.parameters(), lr=self.lr, weight_decay=self.reg)
    ...
    def loss_fn(self, pred, label):
        ...
        elif self.loss == "mse_ic2":
            alpha, beta = self.alpha, self.beta if self.alpha and self.beta else (1.0, 0.1)
            mse = self.mse(p, y)
            ic = ic_loss(p, y)
            return alpha * mse + beta * ic * ic  # IC 损失平方，放大 IC 的影响
        elif self.loss == "mse_ic3":
            alpha, beta = self.alpha, self.beta if self.alpha and self.beta else (1.0, 0.1)
            mse = self.mse(p, y)
            ic = ic_loss(p, y)
            return alpha * mse + beta * ic * ic * ic
        elif self.loss == 'ic':
            return ic_loss(p, y)
        elif self.loss == "mse":
            return self.mse(p, y)
    ...
    def fit(
        ...
        self.fitted = True
        scheduler = StepLR(self.train_optimizer, step_size=10, gamma=0.1)
