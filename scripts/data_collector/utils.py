...
def get_us_stock_symbols(qlib_data_path: [str, Path] = None) -> list:
    ...
    def _get_eastmoney():
        """Get symbols from Eastmoney API with graceful error handling"""
        try:
            url = "http://4.push2.eastmoney.com/api/qt/clist/get?pn=1&pz=10000&fs=m:105,m:106,m:107&fields=f12"
            resp = requests.get(url, timeout=30)
            if resp.status_code != 200:
                logger.warning(f"Eastmoney API returned status code: {resp.status_code}")
                return []

            try:
                data = resp.json()
                if not data.get("data") or not data["data"].get("diff"):
                    logger.warning("Eastmoney API returned unexpected data structure")
                    return []
                
                _symbols = [_v["f12"].replace("_", "-P") for _v in data["data"]["diff"].values()]
                logger.info(f"Retrieved {len(_symbols)} symbols from Eastmoney")
                return _symbols
            except Exception as e:
                logger.warning(f"Failed to parse Eastmoney response: {e}")
                return []
