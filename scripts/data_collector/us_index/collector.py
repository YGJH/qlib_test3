...
class WIKIIndex(IndexBase):
    ...
    def calendar_list(self) -> List[pd.Timestamp]:
        ...
        if _calendar_list is None:
            ...
            if bench_start.tz is not None:
                bench_start = bench_start.tz_localize(None)
            
            calendar_list = get_calendar_list("US_ALL")
            # Normalize all calendar timestamps to be timezone-naive
            calendar_list = [x.tz_localize(None) if x.tz is not None else x for x in calendar_list]
            
            _calendar_list = list(filter(lambda x: x >= bench_start, calendar_list))
